...
@register_criterion("OrderedPccRateDistortionLoss")
class OrderedPccRateDistortionLoss(nn.Module):
    """Custom rate distortion loss with a Lagrangian parameter."""

    def __init__(self, lmbda=5.0, metric="mse"):
        super().__init__()
        if metric == "mse":
...
class ChamferPccRateDistortionLoss(nn.Module):
    ...
    def rec_loss(self, output, target):
        ...
        return loss_chamfer
        # return self.metric(output["x_hat"], target)
